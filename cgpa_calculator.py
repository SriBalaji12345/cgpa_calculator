...
total_semesters = st.number_input(
    "Total number of semesters completed (1-8):",
...
for sem in range(1, total_semesters + 1):
    ...
    num_subjects = st.number_input(
        ...
        min_value=1, step=1, key=f"subjects_{sem}"
    )
    ...
    for subj in range(1, num_subjects + 1):
        ...
        credit = st.number_input(
            f"Credit points for Subject {subj}:",
            min_value=0.0, step=0.5, key=f"credit_{sem}_{subj}"
        )
        grade = st.number_input(
            f"Grade point you gained (0–10):",
            min_value=0, max_value=10, step=1, key=f"grade_{sem}_{subj}"
        )
